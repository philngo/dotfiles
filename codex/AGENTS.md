...
2. Describe before editing. Run `jj describe -m "..."` before making changes, then verify the final diff still matches the description.
...
5. Finish with an empty working copy. Run `jj new` when done so `@` is empty for the next session.
